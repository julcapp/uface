                        ...
                        <>
                            {
                                !(typeof cameraEnable === "number" && cameraEnable === photoIndex) ? (
                                    <div>
                                        <img className={css.image} src={photo.base64 || photoExample} alt="nophoto" />
                                    </div>
                                ) : (
                                    <div>
                                        <video className={css.video} ref={camera} width="340" height="340" autoplay></video>
                                    </div>
                                )
                            }
                                    ...
                                    !(typeof cameraEnable === "number" && cameraEnable === photoIndex) ? (
                                        <>
                                            <button
                                                ...
                                                onClick={async () => {
                                                    setLoadingData(true)
                                                    await request.MakeExtImgAsMain('persident/folkimgext?faceid=' + photo.faceid)
                                            ...
                                            <button
                                                onClick={async () => {
                                                    try {
                                                        const data = await request.deletePhoto(`persident/folkimgext?faceid=${photo.faceid}`)
                                        ...
                                        <>
                                            <h1>{seconds}</h1>
                                            <button className={css.update} onClick={() => shot(photoIndex)}>
                                                Сфотографироваться
                                                        </button>
                                        </>
                                    )
